...
The same engine ships as a `chute` command for people who prefer a terminal to a right-click.
It is free and MIT, like everything here, and it is the honest way to evaluate this project:
read the source, run it, and decide before you trust an app built on top of it.

**→ [Every command, with examples](docs/CLI.md)**

```bash
brew install avaluev/tap/chute   # the terminal tool only — not the app
```
