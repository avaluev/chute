...
export function CopyLine({ text }: { text: string }) {
  ...
  return (
    ...
    >
      ...
      <code className="min-w-0 flex-1 overflow-x-auto whitespace-pre font-[family-name:var(--font-mono-loaded)] text-sm text-foreground [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        <span className="select-none text-muted-foreground">$ </span>
        {text}
      </code>
      <span className="shrink-0 rounded border border-border px-2 py-1 font-[family-name:var(--font-mono-loaded)] text-xs uppercase tracking-wide text-muted-foreground transition-colors group-hover:border-[var(--color-accent-chute)] group-hover:text-[var(--color-accent-chute)]">
        {copied ? "copied" : "copy"}
      </span>
    </button>
  );
