 ...
 *
 * `trailingSlash: true` in next.config.ts means Cloudflare Pages serves `/docs/`, not `/docs.html`.
 * The URLs below must match that or every one of them is a redirect Google has to follow.
 */
const STATIC = ["", "cases", "cli", "docs", "changelog", "building-with-agents",
...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  return [
    ...STATIC.map((p) => ({
      ...
      priority: p === "" ? 1 : ["cases", "cli", "docs"].includes(p) ? 0.8 : 0.3,
    })),
    ...CASES.map((c) => ({
