...
> **HISTORICAL — a record of the eleven days Chute had a price (2026-08-28 to 2026-09-08).**
> Chute is free and MIT. Nothing below is current pricing; it is kept because the reasoning is
> real and deleting it would make this file lie about what was decided and why.
...
> Note (2026-09-08): written while Chute was $19 with a 14-day trial and a licence key
> (`License.swift`). Chute is free and MIT since this date — no price, no key, no trial — so the
> break-even arithmetic below and the licence-gating it describes no longer apply; there is
> nothing left to break even against. The Gatekeeper/notarization mechanics are unchanged by
> price and are kept as live technical reference; the economic argument is kept as the record of
> the decision that was made at the time.
...
### Could the app, back when it was paid, have shipped the same way?
...
- `Sources/ChuteCore/License.swift` gated the app's paid features on an offline Ed25519 signature,
  until the licence check was deleted on 2026-09-08. **The product was monetised by the key, not
  by the binary.** Anyone could already build it; only a key made it keep working past the trial.
...
| Price of Chute at the time | **$19 one-time** — retired 2026-09-08; it is free and MIT now |
