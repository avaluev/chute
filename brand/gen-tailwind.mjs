...
:root {
  ...
  /* THE next/font FACE FIRST, THEN THE TOKEN'S OWN STACK AS FALLBACK.
     ...
     --font-mono-loaded and puts both on <html> — NOT <body>, and that placement is load-bearing:
     these tokens are declared at :root, so a var() consumed here resolves against <html>. With
     the classes on <body> the var was undefined one level up, which makes the WHOLE declaration
     guaranteed-invalid, and the entire site rendered its body copy in Times on 2026-09-09. If
     you move them back to <body>, you reintroduce that bug. The mono variable was consumed all over the
